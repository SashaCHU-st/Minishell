...
/*   Updated: 2024/07/08 17:12:14 by aheinane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (access(shell->cmds[0].filenames[0], F_OK | R_OK) == -1)
	{
		perror("No access for input");
		return (1);
	}
	pipex->fd_in = open(shell->cmds[0].filenames[0], O_RDONLY);
	if (pipex->fd_in == -1)
	{
...
{
	pipex->fd_out = open(shell->cmds[1].filenames[0], O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (pipex->fd_out == -1)
	{
